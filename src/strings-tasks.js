...
function getStringLength(value) {
  return value === undefined || value == null ? 0 : value.length;
}
...
function isPalindrome(str) {
  return (
    str.toLowerCase().match(/[a-z]/gi).join('') ===
    str.toLowerCase().match(/[a-z]/gi).reverse().join('')
  );
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i].charCodeAt(0) >= 65 && arr[i].charCodeAt(0) <= 77) {
      ...
      arr[i] = String.fromCharCode(arr[i].charCodeAt(0) - 13);
    } else if (arr[i].charCodeAt(0) >= 97 && arr[i].charCodeAt(0) <= 109) {
      arr[i] = String.fromCharCode(arr[i].charCodeAt(0) + 13);
    } else if (arr[i].charCodeAt(0) >= 110 && arr[i].charCodeAt(0) <= 122) {
      arr[i] = String.fromCharCode(arr[i].charCodeAt(0) - 13);
    }
  }
  return arr.join('');
